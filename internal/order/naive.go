...
func PlaceOrderNaive(ctx context.Context, db *sqlx.DB, req PlaceOrderRequest) (*Order, error) {
	panic("TODO: phase p1 - 无锁无事务地扣库存 + 插订单，用来复现超卖")
}

// AI 将在 p1 学习时分切片实现（下面是思路与顺序，不是终稿代码）：
// 1. if req.Quantity <= 0 { return nil, ErrInvalidQuantity }
// 2. var stock int
//    err := db.GetContext(ctx, &stock, "SELECT stock FROM products WHERE id = ?", req.ProductID)
//    —— sql.ErrNoRows 时返回 ErrProductNotFound，其他 err 直接返回
// 3. if stock < req.Quantity { return nil, ErrInsufficientStock }
